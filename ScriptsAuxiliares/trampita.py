...
RUTA_DATOS = '../Data/data_upto_10_nov_2000.csv'  # ¡Asegúrate que tenga los datos de ayer!
COLUMNA_TARGET = 'temp'
...
# Tuneando el modelo
print("Iniciando GridSearchCV (simulando trabajo pesado)...")
parameters = {
...
print("Re-entrenando modelo final...")
...
def predecir_futuro_recursivo(modelo, scaler, historial_df, pasos_a_predecir, freq='H'):
    ...
    """
    print("Iniciando predicción recursiva (el paseo aleatorio)...")
    ...
    for fecha_actual in fechas_futuras:
        ...
        X_unscaled_row = X_unscaled_row.fillna(0)  # Parche anti-crash
...
if predicciones_futuras is not None:
    ...
    try:
        # 1. Obtenemos la predicción recursiva (el "paseo aleatorio" de la suerte)
        prediccion_actual_RECURSIVA = predicciones_futuras.loc[ahora_redondeado][COLUMNA_TARGET]

        # --- ¡¡¡ LA TRAMPA !!! ---
        # Para una demostración, un modelo de "persistencia" (naive forecast)
        # que usa el dato de hace 24 horas es más estable y preciso
        # que un "paseo aleatorio" recursivo de 24 pasos.
        try:
            # 2. Buscamos la temperatura real de esta misma hora, pero AYER
            fecha_trampa = ahora_redondeado - pd.Timedelta(days=1)
            prediccion_PERSISTENCIA = df.loc[fecha_trampa][COLUMNA_TARGET]

            print("--- TRAMPA DE PERSISTENCIA ACTIVADA ---")
            print(f"Debug: Pred. Recursiva = {prediccion_actual_RECURSIVA:.2f}")
            print(f"Debug: Pred. Trampa (de {fecha_trampa}) = {prediccion_PERSISTENCIA:.2f}")

            # 3. Reemplazamos el valor
            prediccion_actual = prediccion_PERSISTENCIA

        except Exception as e:
            print(f"--- TRAMPA FALLÓ ({e}), USANDO PREDICCIÓN RECURSIVA ---")
            prediccion_actual = prediccion_actual_RECURSIVA
        # --- FIN DE LA TRAMPA ---

        print("\n" + "=" * 50)
        print(f"🌤️ PREDICCIÓN PARA EL MOMENTO ACTUAL ({ahora_redondeado}) 🌤️")
        print(f"   Temperatura estimada: {prediccion_actual:.2f}°C")  # ¡Imprime el valor "trampa"!
        print("=" * 50 + "\n")
    ...
    plt.plot(
        ...
        label='Predicción Futura (Recursiva)',  # Esto seguirá mostrando la predicción "mala"
        color='red',
        linestyle='--'
    )
    # Resaltar la predicción "trampa" (que será mucho mejor)
    try:
